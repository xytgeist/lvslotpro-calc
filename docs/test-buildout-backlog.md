...
- 2026-06-07: **Lounge market CoinGecko usage tracing (code on `test`):** `coingeckoUsageLog.ts` — per-request `[coingeckoUsage]` Edge logs + optional `_debug.coingecko` when `debug_coingecko: true` or `LOUNGE_MARKET_DEBUG_COINGECKO=1`. Client Console log HUD auto-enables flag. **Redeploy `lounge-market-data` on test** before smoke.
