...
- 2026-05-13: **Lounge share + deep link + OG:** **Share** uses `navigator.share` / **clipboard** (`loungeSharePost.js` + `SocialFeed.jsx` / `LoungePostInteractionBar.jsx`); **`AppShell.jsx`** focuses **Home** when `post=` is in the URL; anonymous **deep open** uses `fromPublicLink` then strips `post`. **Rich previews:** share URLs are **`/lounge/p/<uuid>`**; **`vercel.json`** → **`api/lounge-post-og.js`** (Supabase anon + `og:*` / Twitter card + redirect to `/?tab=home&post=`). **`eslint.config.js`**: **`api/**`** = Node. **Cost:** normal Vercel + Supabase read volume only.
