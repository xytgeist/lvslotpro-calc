...
- 2026-05-15: **Lounge comments — interaction row per card:** **`LoungeCommentInteractionBar`** under each comment body (reply + direct-child reply count; repost/like/bookmark inactive placeholders). **`LoungePostCommentThread.jsx`**; **`SocialFeed.jsx`** passes **`loungeReadOnly`**, auth/profile gates, **`onCommentReplyInteraction`**.
