...
- 2026-06-09: **Lounge auto cashtag chart resolution (code on `test`):** caption `$TICKER` auto-attach now resolves via **Yahoo equity lookup first** (real US stock/ETF) before Finnhub/CoinGecko search; search fallback tie-breaks prefer non-tokenized stocks over Ondo/xStock crypto rows. **Redeploy `lounge-market-data`.**
