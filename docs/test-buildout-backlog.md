...
- 2026-06-08: **Lounge thread failure recovery (code on `test`, `24dedcb`):** failed thread publish restores compose with images/GIF/video per part (not text-only draft); partial publish resume via submit-job snapshot. Smoke with mid-upload failure + **Retry**.
...
- 2026-06-08: **Lounge post drafts (server):** migration **`20260608130000_lounge_post_drafts.sql`** — table **`lounge_post_drafts`** (caption, tribes, GIF URL, uploaded **`image_urls`**, max 20/user); client **`loungePostDraftApi.js`**, **`LoungePostDraftsSheet`**, composer **Save draft** + **Drafts (N)**; publish deletes linked draft; video not stored (re-attach). Apply migration on test before smoke.
