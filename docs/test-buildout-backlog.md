...
- [x] Maintain a "known-good on test" smoke pass list
  ...
  - **Manual on test** (required before prod; mirrors `production-rollout-checklist.md` §5 where applicable):
    ...
    21. **Lounge notifications (Phase H1):** apply migrations **`20260522120000_lounge_activity_events_phase_h1.sql`** + **`20260522140000_lounge_activity_events_realtime.sql`** on test — second account **comments** on your post → bell badge appears **while you stay in the app** (not only after refresh); open **Alerts** → row appears, badge clears; tap → post detail (comment focus when reply); **@mention** in post/comment; **follow** → tap opens profile; feed like/comment/post still work. *(Ryan smoke pending.)*
