...
- 2026-06-07: **Fix: comment/reply push — RLS on emit trigger (test PASSED):** **`20260607210000_activity_events_feed_comment_rls_fix.sql`** — `activity_events_on_feed_comment_insert` with `set row_security = off` + direct insert; without it post-owner lookup under RLS no-ops and no push. Ryan **PASSED** on test (comment → Edge Lounge push, @bfrizzle iOS PWA). **Open:** same `row_security` pattern likely needed on like/bookmark/repost/follow/mention emitters (`activity_events_on_post_like_insert`, `activity_events_on_feed_post_insert`, `activity_events_emit_mentions`, etc.) — track in next migration.
