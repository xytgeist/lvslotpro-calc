...
- 2026-05-13: **Lounge share preview polish:** Native share omits **`text`** so iMessage does not duplicate the caption in a second bubble (`shareLoungePostHybrid` + `SocialFeed.jsx`). **`api/lounge-post-og.js`**: caption first in **`og:description`**, then like/comment counts; **`og:site_name`** + favicon / **`apple-touch-icon`** links (match `index.html`) to help the small site icon beside the domain.
