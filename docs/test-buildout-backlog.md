...
- [x] Maintain a "known-good on test" smoke pass list
  ...
  - **Manual on test** (required before prod; mirrors `production-rollout-checklist.md` §5 where applicable):
    ...
    21. **Lounge notifications (Phase H1):** apply migrations **`20260522120000_lounge_activity_events_phase_h1.sql`**, **`20260523120000_lounge_activity_events_repost.sql`**, and **`20260523140000_lounge_activity_events_bookmark.sql`** on test — second account **comments** on your post → bell badge within ~60s poll (or on refresh / reopen); open **Alerts** → row appears, badge clears; tap → post detail (comment focus when reply); **@mention** in post/comment; **follow** → tap opens profile; **plain repost** and **quote repost** on your post → notification row + tap opens repost card; **comment repost** → tap opens comment repost card; **bookmark** on your post or comment → yellow bookmark badge + tap opens post/comment; feed like/comment/post still work. *(Ryan smoke pending.)*
...
- 2026-05-23: **Phase H1 bookmark notifications (SQL):** migration **`20260523140000_lounge_activity_events_bookmark.sql`** — `bookmark` event type + AFTER INSERT triggers on **`post_bookmarks`** and **`feed_comment_bookmarks`** (notify owner on bookmark only, not unbookmark).
