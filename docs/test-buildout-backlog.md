...
- 2026-05-13: **Composer video preview poster:** short clips (`≤60.35s`) run `captureVideoFilePosterObjectUrl` in `loungeVideoUpload.js` before Stream prep so the composer `<video>` gets a JPEG `poster` (scan / black-frame heuristics aligned with `LoungeVideoCropModal`). `SocialFeed.jsx` passes `posterUrl` into `startComposerVideoPrepFromSpec`.
