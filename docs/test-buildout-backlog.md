...
- 2026-05-13: **Lounge OG + share polish:** Native share omits **`text`** (no duplicate iMessage bubble). **`api/lounge-post-og.js`:** **caption + byline + stats** folded into **`og:title`** (`compoundOgTitle`) because **Apple Messages often omits `og:description`** on large-image cards; **`shortcut icon`** → **`/favicon.ico`** first; **JSON-LD** `SocialMediaPosting`; shorter CDN cache on success response. **`og:description`** kept for other clients.
