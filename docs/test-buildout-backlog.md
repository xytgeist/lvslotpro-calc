...
- [ ] **Phase H2 Lounge web push (test build):** Settings push toggle + Edge **`lounge-send-activity-push`** + migration **`20260523160000_lounge_activity_events_push.sql`**. Deploy function, set **`LOUNGE_ACTIVITY_PUSH_SECRET`** + Vault secrets (see function **`README.md`**); smoke **§21b** pending Ryan sign-off.
...
- 2026-05-23: **Phase H2 Lounge web push (client + SQL + Edge):** Settings push toggle wired to **`useWebPushNotifications`** / **`push_subscriptions`**; Edge **`lounge-send-activity-push`** + migration **`20260523160000_lounge_activity_events_push.sql`** (pg_net on **`activity_events`** INSERT). Deploy function + Vault secrets before smoke **§21b**.
