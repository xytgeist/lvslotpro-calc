...
- 2026-06-27: **iOS PWA install banner (`test` code):** PokerNews-style fixed top strip in **`AppShell`** (**`PwaInstallBanner.jsx`**) — **How to add** expands Safari steps; dismiss persists in **`localStorage`**. Hidden when already on Home Screen app. Layout offset via **`--lv-pwa-install-banner-height`** on **`data-edge-scroll-shell`** shells.
