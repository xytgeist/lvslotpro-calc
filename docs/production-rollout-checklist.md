...
- [ ] **Creator fan subs (2026-07-21 promote, `main` @ `c95c6109`):** apply in order on **`jtjgtucumuoswnbauxry`** — use **`npm run db:query:production -- -f …`** (npm treats bare **`-f`** as **`--force`**, not the SQL file flag):
  ...
  Live **`STRIPE_PRICE_FAN_TIER_*`** Edge secrets (five tiers) + Connect-capable Prices; redeploy **`creator-fan-connect`**, **`creator-fan-checkout`**, **`creator-fan-resume-subscription`**, **`stripe-create-portal-session`**, **`stripe-webhook`** (§4). Smoke: Settings **Subscriptions**, profile **SUB** / subscribe modal, cancel + resume, **Creators I support** → profile.
