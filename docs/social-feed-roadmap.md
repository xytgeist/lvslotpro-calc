...
- **Shipped (foreground in-app toast, test build 2026-05-23):** **`push-sw.js`** detects a **focused** app client for Lounge activity pushes → **`lounge-activity-inapp`** message to **`AppShell`** (banner toast) and **skips OS notification**; background/minimized → OS push unchanged. Offers calendar pushes unchanged. **`LoungeActivityInAppToast.jsx`**; badge refreshes on **`lounge-activity-arrived`**.
