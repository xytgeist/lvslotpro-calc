...
function App() {
  ...
  // Phoenix Link States (exactly as before)
  ...
  const chartData = {
    ...
    datasets: [{
      ...
      data: Array.from({ length: 23 }, (_, i) => {
        const c = 1300 + i * 25;
        return getRecommendedWalkAway(c);
      }),
      borderColor: '#f97316',
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context) => `+${Math.round(context.raw)} bets`
        }
      }
    },
  ...
  // Phoenix Link Calculator (full original code)
  return (
    <div className="min-h-screen bg-gray-950 pb-12">
      {/* Hamburger */}
