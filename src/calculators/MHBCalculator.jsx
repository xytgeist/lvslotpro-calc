...
function MHBCalculator({ onBack }) {
  // Main fields
  const [current, setCurrent] = useState(9350)      // JP Meter (Current)
  const [mustHitBy, setMustHitBy] = useState(10000)
  const [denom, setDenom] = useState(1.00)

  // Advanced Settings
  const [overallRTP, setOverallRTP] = useState(91)
  const [meterRise, setMeterRise] = useState(6.666666)
        ...
        <div className="space-y-6">
          <div className="bg-gray-900 p-5 rounded-3xl space-y-5">
            <div>
              ...
              <input type="text" value={current} onChange={(e) => setCurrent(e.target.value.replace(/[^0-9]/g, ''))} onBlur={(e) => setCurrent(parseInt(e.target.value) || 9350)} className="w-full p-4 bg-gray-800 rounded-2xl text-3xl font-bold text-center text-purple-300" />
            </div>
            <div>
              <label className="block text-gray-400 text-xs mb-1">Must Hit By</label>
              <input type="text" value={mustHitBy} onChange={(e) => setMustHitBy(e.target.value.replace(/[^0-9]/g, ''))} onBlur={(e) => setMustHitBy(parseInt(e.target.value) || 10000)} className="w-full p-4 bg-gray-800 rounded-2xl text-3xl font-bold text-center" />
            </div>
            <div>
            ...
            {showAdvanced && (
              <div className="p-5 pt-0 space-y-6 border-t border-gray-800">
                ...
                <div>
                  ...
                  <input type="text" value={meterRise} onChange={(e) => setMeterRise(e.target.value.replace(/[^0-9.]/g, ''))} onBlur={(e) => setMeterRise(parseFloat(e.target.value) || 6.666666)} className="w-full p-4 bg-gray-800 rounded-2xl text-center text-2xl font-bold" />
                </div>
