...
- **Lounge post threads:** feed **Start thread** → **`LoungeThreadComposeSheet.jsx`** (X-style multi-part UI; parts 2+ → **`feed_comments`** as thread parts). **`LoungeComposerMediaToolbar.jsx`** — split image/video/GIF on all lounge composers. **`loungePostSubmitJob.js`** thread submit + failure resume. Migrations **`20260608140000`**–**`20260608190000`** (320 cap, thread parts, drafts, feed repair, part media, quote repost on parts). Glass chrome + scroll pin: **`index.css`** (`.lounge-thread-compose-*-glass`), **`loungeThreadComposeMedia.js`**, **`loungeThreadComposeVideoPrep.js`**, **`loungeCommentLimits.js`**.
