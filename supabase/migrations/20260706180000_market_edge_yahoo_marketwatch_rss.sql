-- Market Edge: Yahoo Finance + MarketWatch RSS (public headline feeds; replaces dead Reuters RSS).
...
cross join (
  values
    ('Yahoo Finance', 'rss', 'https://finance.yahoo.com/news/rssindex', '{"source_label":"Yahoo Finance"}'::jsonb, 240),
    ('MarketWatch Top Stories', 'rss', 'https://feeds.content.dowjones.io/public/rss/mw_topstories', '{"source_label":"MarketWatch"}'::jsonb, 300),
    ('MarketWatch Real-Time', 'rss', 'https://feeds.content.dowjones.io/public/rss/mw_realtimeheadlines', '{"source_label":"MarketWatch"}'::jsonb, 240)
) as v(name, kind, poll_url, api_config, poll_interval_sec)
where a.pipeline = 'market_news'
  and coalesce(nullif(btrim(a.config->>'news_profile'), ''), case when a.slug = 'crypto-edge' then 'crypto' else 'market' end) = 'market'
  and not exists (
