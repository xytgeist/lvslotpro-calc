-- Best Bet of the Hour — hourly single strongest +EV play across all calendar sports.
-- Apply after 20260704260000. Redeploy lounge-odds-poll after apply.

-- ---------------------------------------------------------------------------
-- Odds config: hourly best bet toggle + min EV threshold
-- ---------------------------------------------------------------------------
alter table public.lounge_bot_odds_config
  add column if not exists best_bet_hour_enabled boolean not null default true;

alter table public.lounge_bot_odds_config
  add column if not exists min_best_bet_hour_ev_pct numeric not null default 4
    check (min_best_bet_hour_ev_pct between 2 and 15);

comment on column public.lounge_bot_odds_config.best_bet_hour_enabled is
  'When true, pg_cron best_bet_hour action posts one strongest +EV play per PT hour.';

comment on column public.lounge_bot_odds_config.min_best_bet_hour_ev_pct is
  'Minimum +EV % on $1 stake to qualify for Best Bet of the Hour (default 4).';

update public.lounge_bot_odds_config
set alert_audience = alert_audience || '{"best_bet_hour": "subscribers"}'::jsonb
where not (alert_audience ? 'best_bet_hour');

-- ---------------------------------------------------------------------------
-- Publish log post_kind
-- ---------------------------------------------------------------------------
...
alter table public.lounge_bot_publish_log
  ...
  check (post_kind in (
    ...
    'in_game_edge', 'period_report', 'best_bet_hour'
  ));

-- ---------------------------------------------------------------------------
-- Cron invoke: add best_bet_hour action
-- ---------------------------------------------------------------------------
...
begin
  ...
  if action not in ('poll_edges', 'daily_slates', 'best_bet_hour') then
    raise warning 'invoke_lounge_odds_poll: action must be poll_edges, daily_slates, or best_bet_hour (got %)', p_action;
    return;
  end if;
...
comment on function public.invoke_lounge_odds_poll(text) is
  'pg_cron helper: POST lounge-odds-poll for each running odds_api bot. Actions: poll_edges, daily_slates, best_bet_hour.';

-- Hourly best bet: minute 5 of every hour (PT bucket dedupe in Edge).
do $$
declare
  jid int;
begin
  for jid in select jobid from cron.job where jobname = 'lounge_odds_poll_best_bet_hour'
  loop
...
select cron.schedule(
  'lounge_odds_poll_best_bet_hour',
  '5 * * * *',
  $$select public.invoke_lounge_odds_poll('best_bet_hour');$$
);

-- ---------------------------------------------------------------------------
-- Portal save: best_bet_hour audience + hourly toggles
-- ---------------------------------------------------------------------------
...
begin
  ...
  if p_patch ? 'alert_audience' then
    ...
    loop
      if v_key not in ('coffee_covers', 'edge', 'line_movement', 'in_game_edge', 'period_report', 'best_bet_hour') then
  ...
  if p_patch ? 'best_bet_hour_enabled' or p_patch ? 'min_best_bet_hour_ev_pct' then
    if v_row.pipeline <> 'odds_api' then
      raise exception 'best bet hour settings apply to odds_api bots only';
    end if;

    if p_patch ? 'min_best_bet_hour_ev_pct' then
      v_hour_ev := (p_patch->>'min_best_bet_hour_ev_pct')::numeric;
      if v_hour_ev is null or v_hour_ev < 2 or v_hour_ev > 15 then
        raise exception 'min_best_bet_hour_ev_pct must be between 2 and 15';
      end if;
    end if;

    update public.lounge_bot_odds_config
    set
      best_bet_hour_enabled = case
        when p_patch ? 'best_bet_hour_enabled' then (p_patch->>'best_bet_hour_enabled')::boolean
        else best_bet_hour_enabled
      end,
      min_best_bet_hour_ev_pct = case
        when p_patch ? 'min_best_bet_hour_ev_pct' then round((p_patch->>'min_best_bet_hour_ev_pct')::numeric, 2)
        else min_best_bet_hour_ev_pct
      end
    where bot_user_id = p_user_id;
...
-- ---------------------------------------------------------------------------
-- Portal snapshot: expose best bet hour settings
-- ---------------------------------------------------------------------------
        ...
        select jsonb_build_object(
          ...
          'odds_config', (
            select jsonb_build_object(
              ...
              'best_bet_hour_enabled', o.best_bet_hour_enabled,
              'min_best_bet_hour_ev_pct', o.min_best_bet_hour_ev_pct,
              'enabled', o.enabled
            )
            from public.lounge_bot_odds_config o
