-- Arb Watch — cross-book arbitrage alerts on poll_edges (post only when arb >= 3%).
-- Apply after 20260704270000. Redeploy lounge-odds-poll after apply.

alter table public.lounge_bot_odds_config
  add column if not exists arb_watch_enabled boolean not null default true;

alter table public.lounge_bot_odds_config
  add column if not exists min_arb_profit_pct numeric not null default 3
    check (min_arb_profit_pct between 1 and 10);

alter table public.lounge_bot_odds_config
  add column if not exists max_arb_alerts_per_day integer not null default 6
    check (max_arb_alerts_per_day between 1 and 24);

comment on column public.lounge_bot_odds_config.arb_watch_enabled is
  'When true, poll_edges scans for cross-book arbs and posts only when profit clears min_arb_profit_pct.';

comment on column public.lounge_bot_odds_config.min_arb_profit_pct is
  'Minimum guaranteed arb profit % on balanced $100 stake (default 3).';

update public.lounge_bot_odds_config
set alert_audience = alert_audience || '{"arb_watch": "subscribers"}'::jsonb
where not (alert_audience ? 'arb_watch');
...
alter table public.lounge_bot_publish_log
  ...
  check (post_kind in (
    ...
    'in_game_edge', 'period_report', 'best_bet_hour', 'arb_watch'
  ));

-- Extend portal save validation for arb_watch audience key.
...
-- Portal snapshot: expose arb watch settings.
        ...
        select jsonb_build_object(
          ...
          'odds_config', (
            select jsonb_build_object(
              ...
              'max_arb_alerts_per_day', o.max_arb_alerts_per_day, 'enabled', o.enabled
            )
