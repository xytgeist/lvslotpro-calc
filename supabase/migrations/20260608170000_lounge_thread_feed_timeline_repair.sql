-- Repair: remove legacy thread continuation rows from the feed timeline.
-- Parts 2+ belong on feed_comments (is_thread_part); only the root post is feed-visible.

delete from public.community_feed_posts
where thread_root_id is not null;

-- Re-assert feed page RPC hides continuations (safe if 20260608140000 already applied).
