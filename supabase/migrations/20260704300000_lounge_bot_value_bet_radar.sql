-- Value Bet Radar — snackable 2–3 strongest +EV plays every ~30 min during peak hours.
-- Apply after 20260704290000. Redeploy lounge-odds-poll after apply.

alter table public.lounge_bot_odds_config
  add column if not exists value_bet_radar_enabled boolean not null default true;

alter table public.lounge_bot_odds_config
  add column if not exists min_value_bet_radar_ev_pct numeric not null default 3.5
    check (min_value_bet_radar_ev_pct between 2 and 15);

alter table public.lounge_bot_odds_config
  add column if not exists max_value_bet_radar_posts_per_day integer not null default 20
    check (max_value_bet_radar_posts_per_day between 1 and 48);

comment on column public.lounge_bot_odds_config.value_bet_radar_enabled is
  'When true, pg_cron value_bet_radar posts 2–3 strongest +EV plays per PT half-hour during peak hours (8am–10pm PT).';

update public.lounge_bot_odds_config
set alert_audience = alert_audience || '{"value_bet_radar": "all"}'::jsonb
where not (alert_audience ? 'value_bet_radar');
...
alter table public.lounge_bot_publish_log
  ...
  check (post_kind in (
    ...
    'in_game_edge', 'period_report', 'best_bet_hour', 'arb_watch', 'sharp_report', 'value_bet_radar'
  ));
...
comment on function public.invoke_lounge_odds_poll(text) is
  'pg_cron helper: POST lounge-odds-poll for each running odds_api bot. Actions: poll_edges, daily_slates, best_bet_hour, value_bet_radar.';

do $$
declare
  jid int;
begin
  for jid in select jobid from cron.job where jobname = 'lounge_odds_poll_value_bet_radar'
  loop
...
select cron.schedule(
  'lounge_odds_poll_value_bet_radar',
  '5,35 * * * *',
  $$select public.invoke_lounge_odds_poll('value_bet_radar');$$
);
