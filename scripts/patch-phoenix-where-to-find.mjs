/**
 * One-off: patch phoenix-link Where to find on test DB (form-first — no repo guide.md).
...
**Summary:** Phoenix Link is currently one of the most widely distributed must-hit-by games in the country. East Coast markets (Atlantic City and Pennsylvania) and Florida have the strongest placement outside of Las Vegas. Availability is excellent and it is actively being installed in new casinos.`
...
const { data, error } = await sb
  ...
  .eq('slug', 'phoenix-link')
  .maybeSingle()

if (error) throw new Error(error.message)
if (!data) throw new Error('phoenix-link not found on test')

let md = data.content_markdown || ''

// Replace existing Where to find block if present
...
console.log('OK phoenix-link Where to find patched on test')
